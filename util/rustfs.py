...
def download_resource(resource_key: str, force: bool = False) -> str:
    ...
    对于 TIFF 文件，会同时下载相关的辅助文件（.tfw, .aux.xml, .ovr）。
    ...
    for file_key in files_to_download:
        ...
        if os.path.exists(file_cached_path) and not force:
            continue
        try:
    ...
    if not os.path.exists(cached_path):
        raise FileNotFoundError(f"RustFS resource download failed: {normalized_key}")
