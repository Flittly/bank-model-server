...
def download_resource(resource_key: str, force: bool = False) -> str:
    ...
    if normalized_key.lower().endswith(".tif") or normalized_key.lower().endswith(
        ".tiff"
    ):
        tiff_base = normalized_key.rsplit(".", 1)[0]
        files_to_download.extend(
            [f"{tiff_base}.tfw", f"{normalized_key}.aux.xml", f"{normalized_key}.ovr"]
        )
    ...
    for file_key in files_to_download:
        ...
        try:
            client.download_file(
                get_rustfs_bucket(), get_remote_object_key(file_key), file_cached_path
            )
...
def extract_tiff_bounds(tiff_path: str) -> dict:
    """
    提取 TIFF 文件的地理边界。

    使用 GDAL 读取 TIFF 文件的地理变换参数和尺寸，计算边界范围。
    返回原始坐标（不转换），由数据库端使用 PostGIS 进行坐标转换。

    Args:
        tiff_path: TIFF 文件的本地路径
    ...
    Raises:
        ...
        RuntimeError: GDAL 无法打开文件
    """
    try:
        from osgeo import gdal, osr
    except ImportError:
        raise RuntimeError("GDAL is not installed. Install with: pip install gdal")
    ...
    if dataset is None:
        raise RuntimeError(f"Failed to open TIFF file: {tiff_path}")
    ...
    try:
        ...
        srid = int(source_srs.GetAuthorityCode(None) or "4326")
...
def get_or_extract_tiff_bounds(
    tiff_key: str, segment: str = None, timepoint: str = None
) -> dict:
    """
    获取 TIFF 边界信息，优先从数据库查询，不存在则提取并存储。

    Args:
        tiff_key: TIFF 文件的资源键，如 'tiff/Mzs/2023/standard/202304/202304.tif'
        segment: 河段标识符（可选，用于提取时）
        timepoint: 时间点（可选，用于提取时）
    ...
    Raises:
        FileNotFoundError: TIFF 文件不存在
    """
    from util import db_ops

    # 先从数据库查询
    bounds = db_ops.get_tiff_bounds(tiff_key)
    if bounds:
        return bounds

    # 数据库没有，需要提取
    tiff_path = resolve_tiff_path(tiff_key, segment=segment or "", timepoint=timepoint)
    bounds = extract_tiff_bounds(tiff_path)

    # 解析 region_code 和 year
    parts = tiff_key.split("/")
    region_code = parts[1] if len(parts) > 1 else None
    year = parts[2] if len(parts) > 2 else None
    timepoint_val = parts[4] if len(parts) > 4 else None

    # 存储到数据库
    db_ops.save_tiff_bounds(
        tiff_key=tiff_key,
